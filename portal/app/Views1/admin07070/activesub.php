...
<section class="content">
      <div class="row">
          ...
          <div class="box">
            ...
            <form class="pull-right" method="POST" action="<?php echo base_url(); ?>download/export/package_sub/status">
              ...
              <table id="example1" class="table table-bordered table-striped">
				...
				<?php 
				$n = 1;
				$row = $this->admin_model->getAllUsersActivePay();  foreach($row as $rowArray){  
        ...
        $refPaid = $this->gfa_model->getOnboardIndividual($rowArray['email']);
				?>
                  <tr class="myTable<?php //echo $rowArray['id'] ?>">
                    ...
                    <td>
                       ...
                       <?php  }  ?>
                        <?php if(!empty($this->gfa_model->getAllDcdtByEmailRef($rowArray['email']))) { ?>
                        <strong>Company:</strong> <?php echo strtoupper($this->gfa_model->getAllDcdtByEmailRef($rowArray['email'])[0]['ref']); ?>
                        <?php }else{ echo '';}  ?>
                        <?php  }else{ ?>
