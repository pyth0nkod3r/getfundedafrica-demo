          ...
          <div class="box">
            ...
            <form class="pull-right" method="POST" action="<?php echo base_url(); ?>download/export/wp_event/">
              ...
              <table id="example1" class="table table-bordered table-striped">
				...
				<?php 
				$n = 1;
				$row = $this->admin_model->getAllGFAConnections();  foreach($row as $rowArray){  
				...
				?>
                  <tr class="myTable<?php //echo $rowArray['id'] ?>">
                    ...
                    <td><?php
                    if($this->admin_model->getAllInvestorNByEmail($rowArray['email'])[0]['Contact_Name'] !=''){
                    echo '<b>'.$this->admin_model->getAllInvestorNByEmail($rowArray['email'])[0]['Contact_Name'].'</b><br>';
                    echo $this->admin_model->getAllInvestorNByEmail($rowArray['email'])[0]['Investor_Name'].'<br>';
                    echo $rowArray['email'].'<br>';
                    echo 'Investor';
                    }else{ echo '';}
                    if($this->admin_model->getAllCorperateByEmail($rowArray['email'])[0]['Key_contact_name'] !=''){
                        echo '<b>'.$this->admin_model->getAllInvestorNByEmail($rowArray['email'])[0]['Key_contact_name'].'</b><br>';
                        echo $this->admin_model->getAllInvestorNByEmail($rowArray['email'])[0]['Corporate_Name'].'<br>';
                        echo $rowArray['email'].'<br>';
                        echo 'Corporate';
                        }else{ echo '';}
                        if($this->admin_model->getAllMentorByEmail($rowArray['email'])[0]['Mentor_name'] !=''){
                            echo '<b>'.$this->admin_model->getAllMentorByEmail($rowArray['email'])[0]['Mentor_name'].'</b><br>';
                            echo $this->admin_model->getAllMentorByEmail($rowArray['email'])[0]['Company'].'<br>';
                            echo $rowArray['email'].'<br>';
                            echo 'Mentor';
                            }else{ echo '';}
                    ...
                    ?> </td>
                    <td>
                    <?php echo '<b>'.$this->admin_model->getAllStartUpNByEmail($rowArray['email_startup'])[0]['Primary_Contact_Name'].'</b><br>' ?> 
                    <?php echo $this->admin_model->getAllStartUpNByEmail($rowArray['email_startup'])[0]['Startup_Company_Name'].'<br>' ?>
