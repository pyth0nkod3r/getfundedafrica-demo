        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <div class="box-header">
              <h3 class="box-title"><?php echo $this->admin_model->getEventsPostById($id)[0]['title'] ?></h3>
            </div>
            <!-- /.box-header -->
            
            <div class="box-body">
            <form class="pull-right" method="POST" action="<?php echo base_url(); ?>download/export/wp_event/wp_id">
                <input type="hidden" name="column_value" value="<?= intval($id) ?>" >
              ...
              <table id="example1" class="table table-bordered table-striped">
				...
				<?php 
				$n = 1;
				$row = $this->admin_model->getAllWpEventById($id);  foreach($row as $rowArray){  
				...
				?>
                  <tr class="myTable<?php //echo $rowArray['id'] ?>">
                    ...
                    <td><?php echo $n++ ?></td>
                    <td><?php if($rowArray['attend_name'] !=''){ echo $rowArray['attend_name'];}else{
                    echo $this->admin_model->getAllStartUpNByEmail($rowArray['email'])[0]['Primary_Contact_Name']; 
                    }
                    ?></td>
                    <td><?php echo $rowArray['attend_gender'] ; ?> </td>
                    <td><?php echo $rowArray['email'] ; ?> </td>
                    <td>
                    <?php echo $rowArray['attend_phone']; ?>
                    </td>
                    
                    <td><?php echo $rowArray['time_submit']; ?></td>
				
                    <td class="display<?php echo $rowArray['id'] ?>">
                    <!-- <a href="<?php echo base_url(); ?>admin/sendeventrequest/<?php echo $rowArray['id']?>"><i class="fa fa-fw fa-external-link"></i> Send More Info  </a>  -->
...
<script type="text/javascript">
  ...
  $(this).closest('tr').remove();
    ...
    $.ajax({
     data:{id:id},
     type: "POST",
     url: "<?php echo base_url(); ?>admin/deleteEventAttendee",
